...
def plot_only_waypoints(og_waypoints: t.List[t.Any], new_waypoints: t.List[t.Any], plots_dir: str):
    """
    Plot original and intermediate waypoints in 3D.

    Args:
        og_waypoints: List of PointWithOrientation (.x, .y, .z) for original waypoints.
        new_waypoints: List of PointWithOrientation (.x, .y, .z) for all waypoints (original + intermediate).
        plots_dir: Output path.
    """
    import matplotlib.pyplot as plt
    import os
    ...
    # --- All waypoints (blue) ---
    for j, p in enumerate(new_waypoints):
        ax.scatter(p.x, p.y, p.z, color='blue', s=10, alpha=0.7, label="Intermediate Waypoints" if j == 0 else "")

    # --- Original waypoints on top (red + labeled) ---
    for j, p in enumerate(og_waypoints):
        ax.scatter(p.x, p.y, p.z, color='red', s=50, label="Original Waypoint" if j == 0 else "")
        ax.text(p.x, p.y, p.z, f"W{j+1}", fontsize=9, color='black')

    ax.set_title("Original + Intermediate Waypoints (3D)")
    ax.set_xlabel("X [m]")
    ax.set_ylabel("Y [m]")
    ax.set_zlabel("Z [m]")
    ax.grid(True)
    ...
    output_path = os.path.join(plots_dir, "plot_only_waypoints", "waypoints_3d_only.png")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()

def plot_3d_ee_trajectory(
    data: dict,
    waypoints_in_3d: list,
    plots_dir: str,
    extended_waypoints_in_3d: t.Optional[list] = None
):
    import os
    import matplotlib.pyplot as plt
    from mpl_toolkits.mplot3d import Axes3D
    ...
    colors = ["#fe6100", "#785ef0", "#dc267f", "#ffb000"]
    ...
    for mode in mode_labels:
        ...
        ax.plot(ee[mask, 0], ee[mask, 1], ee[mask, 2],
                label=mode, color=colors[mode_labels.tolist().index(mode) % len(colors)])

    # Intermediate waypoints: blue (excluding original)
    if extended_waypoints_in_3d is not None:
        extended_set = set(extended_waypoints_in_3d)
        for p in extended_waypoints_in_3d:
            if p not in waypoints_in_3d:
                ax.scatter(p.x, p.y, p.z, color='blue', s=10, alpha=0.7)#, label="Intermediate Waypoint")

    # Original waypoints: red + labeled
    ...
    output_path = os.path.join(plots_dir, "plot_3d_ee_trajectory", f"trajectory_3d_merged.png")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()
...
def plot_2d_ee_projections(
    data: dict,
    waypoints_3d: t.List[t.Any],
    plots_dir: str,
    extended_waypoints_3d: t.Optional[t.List[t.Any]] = None
):
    import os
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    fig.suptitle("End-Effector 2D Projections", fontsize=16)
    ...
    for i, (x_idx, y_idx, xlabel, ylabel, title) in enumerate(projections):
        ...
        for j, mode in enumerate(mode_labels):
            if mode == "undefined":
                continue
            mask = modes == mode
            ax.plot(ee[mask, x_idx], ee[mask, y_idx],
                    label=mode, color=colors[j % len(colors)])

        # Intermediate waypoints: blue
        if extended_waypoints_3d:
            for p in extended_waypoints_3d:
                x_val = getattr(p, ["x", "y", "z"][x_idx])
                y_val = getattr(p, ["x", "y", "z"][y_idx])
                if p not in waypoints_3d:
                    ax.scatter(x_val, y_val, color='blue', s=10, alpha=0.7)#, label="Intermediate Waypoint")

        # Original waypoints: red + labeled
        for j, p in enumerate(waypoints_3d):
            x_val = getattr(p, ["x", "y", "z"][x_idx])
            y_val = getattr(p, ["x", "y", "z"][y_idx])
            ax.scatter(x_val, y_val, color='red', s=50)#, label="Original Waypoint" if j == 0 else "")
    ...
    output_path = os.path.join(plots_dir, "plot_2d_ee_trajectory", f"trajectory_2d_merged.png")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, dpi=300)
    plt.close()
    return output_path
...
def plot_joint_trajectories_merged(data, robot, plots_dir):
    ...
    for joint_idx in range(num_joints):
        ...
        for i, label in enumerate(np.unique(mode)):
            mask = mode == label
            if np.sum(mask) < 10:
                continue
            ...
            axs[0].plot(local_time, joint_pos, label=label, color=colors[i % len(colors)])
            axs[1].plot(local_time, joint_vel, label=label, color=colors[i % len(colors)])
            axs[2].plot(local_time, acc, label=label, color=colors[i % len(colors)])
            axs[3].plot(local_time, jerk, label=label, color=colors[i % len(colors)])
...
def plot_joint_trajectories_all_on_separate_plots(results, robot, plots_dir):
    ...
    spline_colors = ["#fe6100", "#785ef0", "#dc267f", "#ffb000"] 
    ...
    for joint_idx in range(num_joints):
        ...
        for row_idx, entry in enumerate(results):
            ...
            for col_idx, (data_series, ylabel) in enumerate(zip(joint_data, param_labels)):
                ax = axs[row_idx, col_idx] if len(results) > 1 else axs[col_idx]
                ax.plot(time, data_series, color=spline_colors[row_idx])
...
def plot_all_parameters_stacked_by_spline(results, robot, plots_dir):
    num_joints = 7
    param_labels = ["Position [rad]", "Velocity [rad/s]", "Acceleration [rad/s²]", "Jerk [rad/s³]"]
    spline_colors = ["#fe6100", "#785ef0", "#dc267f", "#ffb000"]  # color per planner

    for joint_idx in range(num_joints):
        fig, axs = plt.subplots(4, len(results), figsize=(6 * len(results), 12), sharex='col')
        ...
        for col_idx, entry in enumerate(results):
            mode = entry["planner"]
            color = spline_colors[col_idx % len(spline_colors)]
            time = np.array(entry["time"]) - entry["time"][0]
            pos = np.array(entry["pos"])[:, joint_idx]
            vel = np.array(entry["vel"])[:, joint_idx]
            acc, jerk = calculate_acceleration_jerk(time, vel)
            ...
            for row_idx, (data_series, ylabel) in enumerate(zip(joint_data, param_labels)):
...
def plot_all_joint_trajectories_all_splines_per_one_parameter(results, robot, plots_dir):
    num_joints = 7
    param_labels = ["Position [rad]", "Velocity [rad/s]", "Acceleration [rad/s²]", "Jerk [rad/s³]"]
    param_colors = ["#fe6100", "#785ef0", "#dc267f", "#ffb000"] 
    ...
    for param_idx, (param_label, color) in enumerate(zip(param_labels, param_colors)):
        ...
        for row_idx, entry in enumerate(results):
            ...
            for joint_idx in range(num_joints):
                ...
                ax.plot(time, joint_data, color=param_colors[row_idx], linewidth=1.5)
                ax.grid(True)
...
def plot_joint_trajectories_all_splines_per_one_parameter(results, robot, plots_dir):
    ...
    for joint_idx in range(num_joints):
        for param_idx, (param_label, color) in enumerate(zip(param_labels, param_colors)):
            fig, axs = plt.subplots(1, len(results), figsize=(6 * len(results), 4))
            ...
            for col_idx, entry in enumerate(results):
                ...
                ax = axs[col_idx]
                ax.plot(time, joint_data, color="#fe6100", linewidth=1.5)
                ax.set_title(mode, fontsize=11)
            ...
            fig.suptitle(f"{param_label} – Joint {joint_idx + 1}", fontsize=14, y=0.99)
...
def main():
    ...
    for entry in results:
        ...
        # Compute Cartesian trajectory length
        ee = np.array(entry["ee"])  # shape (N, 3)
        deltas = np.linalg.norm(ee[1:] - ee[:-1], axis=1)
        trajectory_length = np.sum(deltas)
